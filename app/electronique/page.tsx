...
export default function Electronique() {
  ...
  const fetchProducts = async () => {
    setLoading(true)
    let query = supabase.from('products').select('*').eq('category', 'elec')
      ...
      <div style={{
        ...
        {subCats.map((s) => (
          <button key={s.key} onClick={() => setActiveSub(s.key)} style={{
            ...
            borderColor: activeSub === s.key ? '#5C3317' : '#E8D5B0',
            background: activeSub === s.key ? '#5C3317' : 'transparent',
                  ...
                  <div style={{ padding: 12 }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        color: 'white', border: 'none', padding: 8,
                        borderRadius: 6, cursor: p.stock > 0 ? 'pointer' : 'not-allowed',
                        fontWeight: 600, fontSize: 12, fontFamily: 'sans-serif',
                      }}
                    >
